...
import { useNuxt, resolveModule, tryUseNuxt, resolveAlias, useLogger, updateTemplates, isIgnored, hasNuxtModule, extendViteConfig, addTemplate, addServerHandler, addPlugin, installModule, addComponent, addPrerenderRoutes, defineNuxtModule, createResolver, addImports, addServerImports, addTypeTemplate, addVitePlugin } from '@nuxt/kit';
import { hash } from 'ohash';
import { join, normalize, isAbsolute, dirname, extname, relative, resolve } from 'pathe';
...
const I18N_SOURCE_HASH_FIELD = "_i18nSourceHash";
...
function hasLocaleStemIndex(indexes) {
  return (indexes || []).some(
    (idx) => idx.columns.length === 2 && idx.columns[0] === "locale" && idx.columns[1] === "stem"
  );
}
function normalizeLocaleField(extendedSchema, collectionName) {
  const props = extendedSchema?.definitions?.__SCHEMA__?.properties;
  if (props?.locale && props.locale.type !== "string") {
    logger.warn(
      `Collection${collectionName ? ` "${collectionName}"` : ""} declares a non-string \`locale\` field. The i18n integration requires \`locale\` to be a string and is overriding it.`
    );
...
function defineCollection(collection) {
  ...
  let resolvedI18n = collection.i18n;
  if (hasI18nConfig) {
    ...
    if (!i18n.locales.includes(i18n.defaultLocale)) {
      logger.warn(
        `Collection \`i18n\` config has \`defaultLocale: "${i18n.defaultLocale}"\` that is not in \`locales: [${i18n.locales.map((l) => `"${l}"`).join(", ")}]\`. Adding it automatically, declare it explicitly in \`locales\` to silence this warning.`
      );
...
function resolveCollection(name, collection) {
  ...
  return {
    ...
    i18n: collection.i18n === true ? void 0 : collection.i18n
  };
}
    ...
    return [
      ...
      ...await Promise.all(
        publicCollections.map(async (c) => {
          let type = await compile(c.schema, "CLASS").then((code) => code.replace("export interface CLASS", `interface ${pascalCase(c.name)}CollectionItem extends ${parentInterface(c)}`));
          if (c.i18n) {
            type = type.replace(
...
function watchContents(nuxt, options, manifest) {
    ...
    if (matches.length) {
      ...
      for (const match of matches) {
        ...
        if (!localCache || localCache?.checksum !== checksum) {
          ...
          db2.insertDevelopmentCache(keyInCollection, parsedContent, checksum);
        }
        const parsed = JSON.parse(parsedContent);
        if (collection.i18n && parsed?.meta?.i18n) {
          const i18nData = parsed.meta.i18n;
          const sourceLocale = parsed.locale || collection.i18n.defaultLocale;
  ...
  async function broadcast(collection, key, insert) {
    ...
    const keyMatch = (item) => item.includes(`'${escapedKey}',`) || item.endsWith(`'${escapedKey}')`) || item.includes(`id = '${escapedKey}'`);
...
function mergeArraysByIndex(overrideArr, defaultArr) {
  ...
  for (let i = 0; i < maxLen; i++) {
    ...
    } else if (overrideItem !== void 0 && defaultItem !== void 0 && typeof overrideItem === "object" && overrideItem !== null && !Array.isArray(overrideItem) && typeof defaultItem === "object" && defaultItem !== null && !Array.isArray(defaultItem)) {
      result.push(defuByIndex(overrideItem, defaultItem));
    } else {
      result.push(overrideItem !== void 0 ? overrideItem : defaultItem);
    }
  }
  return result;
}
const defuByIndex = createDefu((obj, key, value) => {
  if (Array.isArray(obj[key]) && Array.isArray(value)) {
    obj[key] = mergeArraysByIndex(value, obj[key]);
    return true;
  }
});
function pickSourceByOverride(source, override) {
  if (override && typeof override === "object" && !Array.isArray(override) && source && typeof source === "object" && !Array.isArray(source)) {
    const out = {};
    for (const k of Object.keys(override)) {
      out[k] = pickSourceByOverride(source[k], override[k]);
    }
    return out;
  }
  return source;
}
function expandI18nData(parsedContent, i18nConfig, collectionType, schemaKeys) {
  const meta = parsedContent.meta;
  const i18nData = meta?.i18n;
  if (!i18nData) {
  ...
  for (const [locale, rawOverrides] of Object.entries(i18nData)) {
    ...
    if (!i18nConfig.locales.includes(locale)) {
      logger.warn(
        `Inline i18n in "${parsedContent.id}" defines locale "${locale}" which is not in the collection's locales [${i18nConfig.locales.map((l) => `"${l}"`).join(", ")}]. This translation is ignored.`
      );
      continue;
    }
    const overrides = {};
    ...
    if (schemaKeys) {
      for (const field of Object.keys(overrides)) {
        if (field !== "body" && !schemaKeys.includes(field)) {
          logger.warn(
            `Inline i18n in "${parsedContent.id}" translates "${field}" for locale "${locale}", but "${field}" is not declared in the collection schema. Non-schema fields are not persisted per locale. Declare the field in the collection schema to translate it.`
          );
        }
      }
    }
    ...
    const merged = defuByIndex(overrides, parsedContent);
...
function detectLocaleFromPath(path, stem, i18nConfig) {
  ...
  if (firstPart && i18nConfig.locales.includes(firstPart)) {
    ...
    const strippedPath = pathParts[0] === firstPart ? "/" + pathParts.slice(1).join("/") : path;
    return {
      locale: firstPart,
...
function readI18nModuleOptions(nuxt) {
  const fromOptions = nuxt.options.i18n;
  if (fromOptions?.locales?.length) {
    return fromOptions;
  }
  const moduleEntry = (nuxt.options.modules || []).find(
    (m) => Array.isArray(m) && m[0] === "@nuxtjs/i18n"
  );
...
function resolveI18nConfig(nuxt, collections) {
  ...
  if (hasNuxtModule("@nuxtjs/i18n", nuxt)) {
    ...
    if (!i18nOptions?.locales?.length) {
      ...
      resolvedConfig = {
        locales: localeCodes.includes(i18nOptions.defaultLocale) ? localeCodes : [i18nOptions.defaultLocale, ...localeCodes],
  ...
  for (const [name, collection] of Object.entries(collections)) {
    ...
    if (resolvedConfig) {
      ...
      logger.warn(
        `Collection "${name}" has \`i18n: true\` but ${reason}. Provide an explicit \`i18n: { locales, defaultLocale }\` config or configure @nuxtjs/i18n.`
      );
      collection.i18n = void 0;
    }
  }
}
        ...
        await Promise.all(chunk.map(async (key) => {
          ...
          try {
            ...
            if (collection.i18n && parsedContent?.meta?.i18n) {
              ...
              list.push([keyInCollection, queries, hash2]);
            }
          } catch (e) {
...
export { I18N_SOURCE_HASH_FIELD, module$1 as default, defineCollection, defineCollectionSource, defineContentConfig, defineTransformer, metaStandardSchema, pageStandardSchema, property, z };
