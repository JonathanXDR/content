    ...
    return [
      ...
      ...await Promise.all(
        publicCollections.map(async (c) => {
          let type = await jsonSchemaToTypescriptLite.compile(c.schema, "CLASS").then((code) => code.replace("export interface CLASS", `interface ${scule.pascalCase(c.name)}CollectionItem extends ${parentInterface(c)}`));
          if (c.i18n) {
            type = type.replace(
...
const defuByIndex = defu$1.createDefu((obj, key, value) => {
...
function expandI18nData(parsedContent, i18nConfig, collectionType, schemaKeys) {
  ...
  for (const [locale, rawOverrides] of Object.entries(i18nData)) {
    ...
    const i18nSourceHash = ohash.hash(sourceFields);
...
function resolveI18nConfig(nuxt, collections) {
  ...
  if (kit.hasNuxtModule("@nuxtjs/i18n", nuxt)) {
  ...
  for (const [name, collection] of Object.entries(collections)) {
    ...
    if (resolvedConfig) {
      ...
      collection.fields = schema_js.getCollectionFieldsTypes(collection.extendedSchema);
...
exports.I18N_SOURCE_HASH_FIELD = I18N_SOURCE_HASH_FIELD;
